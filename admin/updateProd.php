...
if (isset($_POST['update'])) {
	...
	if (!empty($_FILES['new_product_image']['name'])) {
		...
		$target_path = "../uploads/";
		$target_path = $target_path . basename($_FILES['new_product_image']['name']);
	
		if (move_uploaded_file($_FILES['new_product_image']['tmp_name'], $target_path)) {
			...
			$stmt->bind_param("siisss", $product_name, $product_price, $product_stock, $product_description, $product_image, $pid);
		} else {
		...
		$stmt->bind_param("siiss", $product_name, $product_price, $product_stock, $product_description, $pid);
	}
        ...
        <section class="content">
            ...
            <div class="border border-secondary card-body font m-auto rounded-3 w-50">
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group mb-3">
                        <label for="new_product_image">Update Product Image</label>
                        <input type="file" name="new_product_image" class="mt-2 border-secondary form-control" />
                    </div>
